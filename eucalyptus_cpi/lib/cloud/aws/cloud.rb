...
module Bosh::EucalytpusCloud

  class Cloud < Bosh::Cloud
    include Helpers

    DEFAULT_MAX_RETRIES = 2
    DEFAULT_AVAILABILITY_ZONE = "open"
    DEFAULT_EC2_ENDPOINT = "ecc.eucalyptus.com:8773/services/Eucalyptus"
    ...
    ##
    # Initialize BOSH Eucalytpus CPI
    ...
    def create_disk(size, instance_id = nil)
      with_thread_name("create_disk(#{size}, #{instance_id})") do
        ...
        if size < 1024
          cloud_error("Eucalytpus CPI minimum disk size is 1 GiB")
        end

        if size > 1024 * 1000
          cloud_error("Eucalytpus CPI maximum disk size is 1 TiB")
        end
    ...
    def validate_options
      unless @options.has_key?("aws") &&
        ...
        raise ArgumentError, "Invalid Eucalytpus configuration parameters"
      end
